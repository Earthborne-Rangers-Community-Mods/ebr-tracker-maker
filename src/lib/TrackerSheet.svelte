<script lang="ts">
  ...
  let { tracker }: { tracker: TrackerState } = $props();
  ...
  const logoTextLines = $derived(tracker.logoText.split(/\r?\n/));

  // Auto-fit the text wordmark: scale it down so it never overflows its fixed
  // slot (and so a long wordmark can never push the sheet past one A4 page).
  // `scrollWidth`/`scrollHeight` ignore the CSS transform, so measuring the
  // natural size is a single deterministic pass.
  let wordmarkEl: HTMLDivElement | undefined = $state();
  let wordmarkScale = $state(1);
  ...
  $effect(() => {
    logoTextLines; // re-measure whenever the wordmark text changes
    fontsReady; // ...and once webfonts finish loading
    const el = wordmarkEl;
    ...
    // Never scale up - only shrink oversized text to fit.
    wordmarkScale = Math.min(
      1,
      slot.clientWidth / naturalW,
      slot.clientHeight / naturalH,
    );
  });
  ...
  function moonLitPath(k: number, r: number): string {
...
<div class="page-frame">
  <section class="sheet" aria-label="Campaign tracker">
    ...
    <div class="day-grid">
      ...
      {#each tracker.weatherGroups as g}
        ...
        <div class="wordmark">
          <div class="wm-fit">
            {#if tracker.logoMode === "text"}
              ...
              >
                {#each logoTextLines as line}
                  <span>{line}</span>
                {/each}
              </div>
            {:else if logoSrc}
              <img class="wm-logo" src={logoSrc} alt="Campaign logo" />
            {/if}
          </div>
...
<style>
  ...
  /* Fixed slot: the wordmark never grows the layout, so it cannot push the
     sheet onto a second page. Text scales down to fit (see the measure effect)
     and images are contained (aspect preserved); overflow is a backstop. */
  .wm-fit {
    width: 148pt;
    height: 34pt;
    display: flex;
